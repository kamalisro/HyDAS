...
import numpy as np
from ZProf import FileName, Link
import os
...
class Ui_RGBDialog1(object):
    ...
    def GS(self):
        ...
        Ui_RGBDialog1.nameGS = str(name)
        self.lineEdit_4.setText(" Band_"+Ui_RGBDialog1.nameGS[-6]+Ui_RGBDialog1.nameGS[-5])
    ...
    def R(self):
        bandpath=str(Path.BandFolderPath)
        name = QtGui.QFileDialog.getOpenFileName(None, 'Select Band', bandpath)
        Ui_RGBDialog1.nameR = str(name)
        self.lineEdit.setText(" Band_"+Ui_RGBDialog1.nameR[-6]+Ui_RGBDialog1.nameR[-5])
    ...
    def G(self):
        bandpath=str(Path.BandFolderPath)
        name = QtGui.QFileDialog.getOpenFileName(None, 'Select Band', bandpath)
        Ui_RGBDialog1.nameG = str(name)
        self.lineEdit_2.setText(" Band_"+Ui_RGBDialog1.nameG[-6]+Ui_RGBDialog1.nameG[-5])
        im = Image.open(str(name))
    ...
    def B(self):
        bandpath=str(Path.BandFolderPath)
        name = QtGui.QFileDialog.getOpenFileName(None, 'Select Band', bandpath)
        Ui_RGBDialog1.nameB = str(name)
        self.lineEdit_3.setText(" Band_"+Ui_RGBDialog1.nameB[-6]+Ui_RGBDialog1.nameB[-5])
        im = Image.open(str(name))
    ...
    def load1(self):
        ...
        bandpath=str(Path.BandFolderPath)
        if (not Ui_RGBDialog1.nameR) or (not Ui_RGBDialog1.nameG) or (not Ui_RGBDialog1.nameB):
            return
        ...
        self.image_display.save(imagepath+"\Image.tif")
        x = 0
        while x<(self.band):
            os.remove(bandpath+"\Band_0"+str(x+1)+".tif")
            x = x+1
        #Ui_RGBDialog1.Diag1.close()
        Ui_RGBDialog1.nameR = ""
        Ui_RGBDialog1.nameG = ""
        Ui_RGBDialog1.nameB = ""
        Ui_RGBDialog1.nameGS = ""
    ...
    def load2(self):
        imagepath = str(Path.TempImagePath)
        bandpath=str(Path.BandFolderPath)
        if (not Ui_RGBDialog1.nameGS):
            return
        self.GreyIm = Image.fromarray(self.Grey)
        self.GreyIm.save(imagepath+"\Image.tif")
        x = 0
        while x<(self.band):
            os.remove(bandpath+"\Band_0"+str(x+1)+".tif")
            x = x+1
